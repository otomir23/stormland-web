...
import { AUTH_COOKIE_NAME, authErrors, generateAccessToken, getOfflineUUIDWithCredentials } from "@/auth"
    ...
    return (
        <Main>
            <OfflineAuthFormWrapper
                className="absolute inset-x-0 mx-auto flex w-full max-w-sm flex-col gap-4 rounded-2xl border-neutral-100
                p-8 sm:static sm:mt-8 sm:border"
                action={credentialsSignIn}
            >
                {error && (
                    <div
                        className="flex items-center gap-4 rounded-lg border border-red-100 bg-red-50 px-4 py-2 text-sm
                        text-red-600"
                    >
